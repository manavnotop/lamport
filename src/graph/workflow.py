...
def create_workflow(test_mode: bool = False) -> StateGraph:
    ...
    # Define edges - linear flow with conditional on clarification
    workflow.set_entry_point("spec_interpreter")

    # Conditional edge from spec_interpreter - check if clarification is needed
    workflow.add_conditional_edges(
        "spec_interpreter",
        should_ask_clarification,
        {
            "clarify": "clarification",
            "proceed": "project_planner",
        },
    )

    # After clarification, re-run spec_interpreter with enriched spec
    workflow.add_edge("clarification", "spec_interpreter")
    ...
    # Edge from feedback - user provided feedback, go back to debugger
    workflow.add_edge("feedback", "debugger")
...
def should_proceed_to_build(state: GraphState) -> str:
    ...
    if state.validation_passed:
        ...
        return "debugger"
    elif state.user_feedback is None:
        return "feedback"
    else:
        return "abort"
...
def should_ask_clarification(state: GraphState) -> str:
    ...
    Returns 'clarify' if spec needs clarification, 'proceed' otherwise.
    """
...
async def clarification_node(state: GraphState) -> GraphState:
    """Ask clarifying questions to the user and update the spec.

    This node handles human-in-the-loop clarification when the spec is vague.
    It asks the questions generated by the spec_interpreter, then enriches
    the user_spec with the answers before re-running the interpreter.
    """
    from rich.console import Console

    if state.on_event:
        state.on_event("clarification:start")
    ...
    if not questions:
        logger.warning("[Clarification] No questions to ask, proceeding anyway")
        return GraphState(
            **state.model_dump(),
    ...
    # In test mode, skip clarification to avoid interactive prompts
    if state.test_mode:
        logger.info("[Clarification] Test mode - skipping user prompts")
        state_dict = state.model_dump()
        # Remove fields we'll override to avoid duplicate keyword args
        for key in [
            "needs_clarification",
        ...
        return GraphState(
            **state_dict,
            needs_clarification=False,
            clarification_answers={"test_mode": "skipped"},
            user_spec=state.user_spec + " (test mode - clarification skipped)",
            retry_count=state.retry_count,
            project_root=state.project_root,
            on_event=state.on_event,
            test_mode=state.test_mode,
    ...
    console.print("\n[bold cyan]I need some clarification:[/bold cyan]\n")
    ...
    # Enrich the user spec with the answers
    enrichment_parts = [state.user_spec]
    for _question, answer in answers.items():
        if answer:
            enrichment_parts.append(f"\n\nUser clarified: {answer}")

    enriched_spec = " ".join(enrichment_parts)

    logger.info(f"[Clarification] Received {len(answers)} answers from user")

    if state.on_event:
        state.on_event("clarification:end")

    return GraphState(
        needs_clarification=False,
        clarification_answers=answers,
        user_spec=enriched_spec,
        retry_count=state.retry_count,
        project_root=state.project_root,
        on_event=state.on_event,
        test_mode=state.test_mode,
...
async def feedback_node(state: GraphState) -> GraphState:
    """Ask user for feedback after debugger exhausts retries.

    This node handles human-in-the-loop feedback when the debugger has
    exhausted MAX_RETRIES. The user can provide additional context about
    the issue that the debugger can use on its next attempt.
    """
    from rich.console import Console

    if state.on_event:
        state.on_event("feedback:start")

    logger.info(f"[Feedback] Debugger exhausted {MAX_RETRIES} retries, asking user for feedback...")

    console = Console()

    console.print("\n[bold red]The debugger was unable to fix the build errors after[/bold red]")
    console.print(f"[bold red]{MAX_RETRIES} attempt(s).[/bold red]\n")

    if state.validation_errors:
        console.print("[bold yellow]Validation Errors:[/bold yellow]")
        for error in state.validation_errors[:5]:
            console.print(f"  [red]•[/red] {error}")
        if len(state.validation_errors) > 5:
            console.print(f"  ... and {len(state.validation_errors) - 5} more errors")

    if state.build_logs:
        console.print("\n[bold yellow]Build Logs (last 500 chars):[/bold yellow]")
        console.print(f"[dim]{state.build_logs[-500:]}[/dim]")

    if state.error_message:
        console.print(f"\n[bold yellow]Error:[/bold yellow] {state.error_message}")

    console.print(
        "\n[bold cyan]Please describe what's wrong or provide additional context:[/bold cyan]"
    )

    if state.test_mode:
        user_input = "test feedback"
        console.print(f"[dim]Test mode: using '{user_input}'[/dim]")
    else:
        user_input = console.input("> ").strip()

    logger.info(f"[Feedback] User provided feedback: {user_input[:100]}...")

    if state.on_event:
        state.on_event("feedback:end")

    return GraphState(
        **state.model_dump(),
        user_feedback=user_input if user_input else None,
        retry_count=0,
        project_root=state.project_root,
        on_event=state.on_event,
        test_mode=state.test_mode,
